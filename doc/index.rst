...
This module also defines the class :py:class:`rcpy.gpio.InputEvent` to
...
.. py:data:: rcpy.gpio.HIGH
	     
   Logic high level; equals `1`.
	     
.. py:data:: rcpy.gpio.LOW
...
.. py:class:: rcpy.gpio.InputTimeout()

   Exception representing an input timeout event.

.. py:class:: rcpy.gpio.Input(pin)
...
.. py:class:: rcpy.gpio.InputEvent(input, event, debounce = 0, timeout = None, target = None, vargs = (), kwargs = {})
   ...
   .. py:method:: action(event, *vargs, **kwargs)
      ...
      :param event: either :py:data:`rcpy.gpio.LOW` or :py:data:`rcpy.gpio.LOW`
...
.. py:function:: rcpy.gpio.set(pin, value)
...
.. py:function:: rcpy.gpio.read(pin, timeout = None)
...
imports the module. The module :py:class:`rcpy.button.Button` provides
...
button before releasing because :py:meth:`gpio.button.released`
...
This module also defines the class :py:class:`rcpy.gpio.ButtonEvent` to
handle input events. For example::
...
Alternatively one could have created an input event handler by passing
a function to the argument `target` of
:py:class:`rcpy.gpio.ButtonEvent` as in::
...
The main difference between the classes in :ref:`rcpy_button` and
:ref:`rcpy_gpio` is that the classes in :ref:`rcpy_button` define the
constants :py:data:`rcpy.button.PRESSED` and
:py:data:`rcpy.button.RELEASED`, the events
:py:data:`rcpy.button.ButtonEvent.PRESSED` and
:py:data:`rcpy.button.ButtonEvent.RELEASED`, and handle debouncing by
default.
...
.. py:class:: rcpy.button.ButtonEvent(input, event, debounce = rcpy.button.DEBOUNCE, timeout = None, target = None, vargs = (), kwargs = {})
   ...
   .. py:data:: PRESSED
      ...
      Event representing pressing a button; equal to :py:data:`gpio.InputEvent.LOW`.
		 
   .. py:data:: RELEASED

      Event representing releasing a button; equal to :py:data:`gpio.InputEvent.HIGH`.
...
imports the module. The module :py:class:`rcpy.button.LED` provides
objects corresponding to the *RED* and *GREEN* buttons on the Robotics
Cape. Those are :py:data:`rcpy.led.red` and
:py:data:`rcpy.led.green`. For example::
...
returns :samp:`True` is the *GREEN* LED is on.

This module also defines the class :py:class:`rcpy.led.Blink` to
...
instantiate a :py:class:`rcpy.led.Blink` object by calling
